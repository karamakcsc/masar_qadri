...
# Safe import to prevent crash if dependency missing
try:
    import barcode
    from barcode.writer import ImageWriter
except ImportError:
...
@frappe.whitelist()
def generate_barcode_py(doc):

    if not barcode:
        frappe.msgprint("Python barcode library is not installed")
        return
    ...
    if doc.items:
        ...
        for item in doc.items:
            ...
            if item.barcode:
                ...
                barcode_image = get_barcode(item.barcode, "code128")

                if barcode_image:
                    item.barcode_text = barcode_image